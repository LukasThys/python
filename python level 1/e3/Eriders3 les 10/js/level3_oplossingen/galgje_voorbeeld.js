...
function kiesWoord(){
  ...
  //  array 'gevondenLetters' maken we leeg om te beginnen
	gevondenLetters = [];
  //   en variabele 'fouteGokkenOver' zetten we op 11
  fouteGokkenOver = 10;
...
function raadLetter(){
  ...
  if (teZoekenLetters.includes(letter) && !gevondenLetters.includes(letter)){
    ...
    if (teZoekenLetters.join("") === gevondenLetters.join("")){
      woordGevonden();
    }
  }
  else{
...
function woordGevonden(){
  ...
  for (let i = 0; i<teZoekenLetters.length; i++ ){
    //  en zet voor elke letter de plaats in gevondenLetters op de juiste letter
		  gevondenLetters[i] = teZoekenLetters[i];
	}
